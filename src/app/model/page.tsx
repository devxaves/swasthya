...
const ModalLayout = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const response = await fetch("http://localhost:4000/api/user/get-top-profiles", {
        method: "POST",
        headers: {
      ...
      const mainData = JSON.parse(data.request);
      const generatedMolecules = JSON.parse(mainData.molecules).map((mol: any) => ({
